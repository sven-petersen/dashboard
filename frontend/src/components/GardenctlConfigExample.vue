...
<template>
  <div>
    ...
    <div class="mt-2">
      ...
      <span class="font-family-monospace">kubeconfig</span>
      which you can download from the <router-link :to="accountRoute">
        Account
      </router-link> page.
    </div>
  </div>
...
const accountRoute = { name: 'Account' }
const store = useStore()

const cfg = computed(() => store.state.cfg)
const gardenctlConfigYaml = computed(() => {
  return `gardens:
  - identity: ${cfg.value.clusterIdentity}
    kubeconfig: "<path-to-garden-cluster-kubeconfig>"`
})
const configCmd = computed(() => {
  return `gardenctl config set-garden ${cfg.value.clusterIdentity} --kubeconfig "<path-to-garden-cluster-kubeconfig>"`
})
</script>
